...
if SERVER then
    ...
    net.Receive("rptools_tool_play_sound", function(_, ply)
        ...
        if ent:GetClass() ~= "ent_rptools_node" then return end
        ...
        if add then
            ...
            for i, action in ipairs(ent.actions) do
                if action.target == targetTarget
                and action.action == targetAction
                and action.sound == sound then
                    actionIndex = i
                    break
                end
            end
...
if CLIENT then
    function TOOL:DrawToolScreen(width, height)
        ...
        if scope == "world" and loop then title = "Loop Sound" end
        
        draw.SimpleText(title, "DermaLarge",
        width / 2, height * 0.2,
        Color(180, 180, 180),
        TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        local soundDisplay = sound == "" and "(no sound)" or sound
        if #soundDisplay > 28 then soundDisplay = "..." .. string.sub(soundDisplay, -25) end
        
        draw.SimpleText(soundDisplay, "DermaDefault",
        width / 2, height * 0.45,
        Color(240, 235, 230),
        TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        local details = "scope: " .. scope
        draw.SimpleText(details, "DermaDefault",
        width / 2, height * 0.65,
        Color(180, 180, 180),
        TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        draw.SimpleText("Configure in menu", "DermaDefault",
        width / 2, height * 0.9,
        Color(120, 120, 120),
        TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    function TOOL.BuildCPanel(panel)
        ...
        stopButton.DoClick = function()
            local s = GetConVar("rptools_play_sound_sound"):GetString()
            if s ~= "" then LocalPlayer():StopSound(s) end
        end
        ...
        cvars.AddChangeCallback("rptools_play_sound_scope", function()
            if IsValid(loopCheckbox) then updateEnabled() end

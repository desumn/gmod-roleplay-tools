...
RPTools.Templating.RegisterTemplate({
  ...
  parameters = {
    ...
    outFlag = param.Flag("Counter name", "Flag that will be set to true", true, "action")
  },
  transformer = SERVER and function(args, context)
    ...
    local action = RPTools.Actions.Server.Create("increment", { key = args.outFlag, value = args.step })

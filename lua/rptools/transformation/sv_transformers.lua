...
local function addDistance(node, min, max)
    if min ~= nil and min < 0 then return nil end
    if min ~= nil and max ~= nil and max < min then return nil end
...
local function addViewAngle(node, maxAngleDegrees)    
    if maxAngleDegrees < 0 or maxAngleDegrees > 180 then return nil end
    ...
    for _, condition in ipairs(node.conditions) do
        if condition.test == "distance" then hasDistance = true end
    end
    
    if not hasDistance then return nil end
...
local function addStateCondition(node, scope, key, equals, notEquals)
    if not (scope == RPTools.State.SCOPE.GLOBAL or scope == RPTools.State.SCOPE.PLAYER) then return nil end
    if (equals == nil and notEquals == nil) then return end
    if equals ~= nil and notEquals ~= nil then return nil end
...
local function addPlayPlayerSound(node, sound, volume, pitch)
    if volume ~= nil and (volume < 0 or volume > 1) then return nil end
    if pitch ~= nil and (pitch < 0 or pitch > 255) then return nil end
    if not file.Exists("sound/" .. sound, "GAME") then return nil end
...
local function addPlayWorldSound(node, sound, volume, pitch, level)
    if volume ~= nil and (volume < 0 or volume > 1) then return nil end
    if pitch ~= nil and (pitch < 0 or pitch > 255) then return nil end
    if level ~= nil and level and (level < 0 or level > 511) then return nil end
    if not file.Exists("sound/" .. sound, "GAME") then return nil end

    ---@type RPToolsWorldAction
    local playSoundAction = { target = "world", action = "play_sound", sound = sound, volume = volume, pitch = pitch, level = level}
...
local function addLoopSound(node, sound, iterations, volume, pitch, level)
    ...
    if level ~= nil and level and (level < 0 or level > 511) then return nil end
    if not file.Exists("sound/" .. sound, "GAME") then return nil end
    if iterations ~= nil and iterations < 0 then return nil end

    ---@type RPToolsWorldAction
    local playSoundAction = { target = "world", action = "loop_sound", iterations = iterations, sound = sound, volume = volume, pitch = pitch, level = level}

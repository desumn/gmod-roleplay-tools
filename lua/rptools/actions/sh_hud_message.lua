if SERVER then
  RPTools.Actions.Server.RegisterClientAction("hud_message", function(params)
    return istable(params) and isstring(params.message)
        and RPTools.Utilities.IsNumber(params.duration)
  end, function(params)
...
hook.Add("HUDPaint", "RPTools_HUD_Message", function ()
    ...
    if table.IsEmpty(messages) then return end
    ...
    draw.SimpleText(message.text, "RPToolsHUDMessageFont", textX + 2, textY + 2, Color(0, 0, 0, alpha * 0.8), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
    
    local couleurTexte = Color(240, 235, 230)
    draw.SimpleText(message.text, "RPToolsHUDMessageFont", textX, textY, ColorAlpha(couleurTexte, alpha), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
end)

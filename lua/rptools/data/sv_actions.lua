...
local function validateType(actionType)
  return RPTools.Utilities.MakeError(
    actionFunctions[actionType] ~= nil,
    "Invalid action type: " .. tostring(actionType)
  )
end
...
local validators = {
  chat_message = function(params)
    return istable(params)
      and isstring(params.message)
      and RPTools.Utilities.AllKeys(params, function(key)
        return key == "message"
      end)
  end,
...
local function validateParams(actionType, params)
  return RPTools.Utilities.MakeError(
    validators[actionType](params),
    "Invalid params for type"
      .. tostring(actionType)
      .. "("
